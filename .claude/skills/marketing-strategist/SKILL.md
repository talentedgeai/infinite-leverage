...
Using this skill's own `templates/strategy.md` (beside this SKILL.md, not the project's top-level `templates/`), fill every field from the transcript + gap answers. Show the draft **inline** (not as a file yet). Ask for edits. Iterate until the user says "ship it" or equivalent.
...
- Use this skill's `templates/content-process.md` and `templates/content-calendar.md`.
...
- `content/content-calendar/content-calendar.md`
...
Use the README contents from this skill's `templates/folder-readmes.md`. Do NOT create example/seed content inside these folders — that's a downstream agent's job.
...
> - `content/content-calendar/content-calendar.md` — week 1 seeded; extend week by week
>
> Next: the **writer** agent extends the calendar week by week and drafts each topic
> (`writer-seo-content` → `writer-quality-critique`). Hand it the calendar path.
...
- **Match the house style already in the repo.** Read an existing doc under `docs/` or
  `context/` before writing and follow its punctuation and formatting conventions rather
