...
**ALL agents, skills, rules, scaffold, and bootstrap prompts live in ONE repo:**
...
**Rules:**
1. The onboarding laptop is a **consumer** of this repo. Never edit agents, skills, or scaffold files locally — change the repo, then run `infiniteleverage-patch` to pull updates.
2. The AGENT-DELEGATION section in every CLAUDE.md (global and project) is generated by `scripts/inject-agent-delegation.sh` — modifications to its content must happen in the canonical repo.
3. The bundled copy inside this skill's zip is an offline fallback only. Prefer fresh `gh repo clone --depth 1` when online.
